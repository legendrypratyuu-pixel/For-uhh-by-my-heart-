...
const App = () => {
  ...
  const exportData = () => {
    ...
    a.href = url; a.download = "love-scrapbook.json"; a.click();
  ...
  // Your images URLs
  const initialPhotos = [
    { src: "https://i.postimg.cc/DJdGr5RP/image1.jpg", caption: "My Love 💖" },
    { src: "https://i.postimg.cc/Q9s9d1V9/image2.jpg", caption: "Memories" },
    { src: "https://i.postimg.cc/Zv5Wrm8J/image3.jpg", caption: "Together" },
    { src: "https://i.postimg.cc/hXkQNHB0/image4.jpg", caption: "Sweet Moments" },
    { src: "https://i.postimg.cc/PCp83dBx/image5.jpg", caption: "Always" },
    { src: "https://i.postimg.cc/rKp441j5/image6.jpg", caption: "Forever" },
    { src: "https://i.postimg.cc/Rqdfqs7M/image7.jpg", caption: "Joy" },
    { src: "https://i.postimg.cc/14mVxGLb/image8.jpg", caption: "Love" },
    { src: "https://i.postimg.cc/6y1vf8bg/image9.jpg", caption: "Happiness" },
    { src: "https://i.postimg.cc/R3wJmXCx/image10.jpg", caption: "Our Journey" },
    { src: "https://i.postimg.cc/gXDwXj6M/image11.jpg", caption: "Forever Us" },
  ];

  // Initialize photos if empty
  useEffect(() => {
    if (photos.length === 0) setPhotos(initialPhotos);
  }, []);
  ...
  return (
    <div className={`min-h-screen font-sans ${themes[theme]} transition-colors duration-400`}>
      <header className={`sticky top-0 z-10 p-4 text-center backdrop-blur-sm ${headerThemes[theme]}`}>
        <h1 className="text-4xl font-bold mb-2" contentEditable>💖 Our Love Scrapbook 💖</h1>
      ...
      <ScrapbookSection title="📸 Memories Gallery">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {photos.map((photo,index) => (
            <motion.div key={photo.src+index} initial={{ opacity:0, scale:0.5 }} animate={{ opacity:1, scale:1 }} exit={{ opacity:0, scale:0.5 }} transition={{ duration:0.3 }} whileHover={{ scale:1.05 }} className={`p-4 rounded-xl shadow-md ${cardThemes[theme]}`}>
        ...
        <AnimatePresence initial={false}>
          <div className="flex flex-col-reverse gap-3">
            {timeline.map((item,index)=>(
              <motion.div key={item.date+index} initial={{opacity:0, y:50}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-50, scale:0.8}} transition={{duration:0.3}} className={`p-4 rounded-xl shadow-md ${cardThemes[theme]}`}>
